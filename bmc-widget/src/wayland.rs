...
fn apply_configure_event(
    ...
    use bmc_widget_protocol::wayland_client::WEnum;
    let shape = match viewport_shape {
        WEnum::Value(v) => Some(v.into()),
        WEnum::Unknown(value) => {
            tracing::warn!(
                value,
                "configure event carries unknown viewport_shape; defaulting to Rectangular"
            );
            None
        }
    }
    .unwrap_or(ViewportShape::Rectangular);
...
fn apply_display_info_event(
    ...
    let Some(shape) = (match shape {
        ...
        WEnum::Unknown(value) => {
            tracing::warn!(
                value,
                "display_info event carries unknown display_shape; ignoring event"
            );
            None
        }
    }) else {
