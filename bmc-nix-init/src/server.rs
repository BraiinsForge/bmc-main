...
fn try_from_grpc_encryption_type(value: GrpcEncryptionType) -> Result<EncryptionType, Status> {
    match value {
        GrpcEncryptionType::Unspecified => Err(Status::invalid_argument(
...
impl<P: InitPlatform + 'static> InitialSetupService for InitSetupService<P> {
    ...
    async fn set_wifi(
        ...
        let encryption = try_from_grpc_encryption_type(req.encryption_type())?;
        let ssid = req.ssid;
