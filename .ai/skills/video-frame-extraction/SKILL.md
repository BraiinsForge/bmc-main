...
ffmpeg -loglevel error -i <path> -vf "fps=15" .ai/scratch/video-frames/<slug>/d_%03d.png
```

Report back: *"Frames at `.ai/scratch/video-frames/<slug>/d_NNN.png`, NNN total. Which frame(s) does the effect show up
at?"*
...
Leave the frames in `.ai/scratch/video-frames/` for the rest of the session — useful if the user asks a follow-up about
a different timestamp. They're gitignored, so they won't be committed; don't `rm -rf` the directory automatically. If
the user wants it gone, they can ask.
