...
//
// Adapted from bmc-openwrt/src/linux_drm_platform.rs for the
// self-contained init binary.
    ...
    fn run_event_loop(&self) -> std::result::Result<(), slint::PlatformError> {
        ...
        'outer: while !self.event_receiver.is_disconnected() {
            ...
            self.window.draw_if_needed(|renderer| {
                ...
                // because Slint renders layers one by one. This behaviour does not work well in DRM
            ...
            // Do not sleep when there are active animations
...
impl LinuxDrmPlatform {
    pub fn new(width: usize, height: usize, rotation: RenderingRotation) -> Self {
