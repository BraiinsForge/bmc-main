...
use bmc_shared_ii_net_drv::wifi::{OpenwrtWifiManager, SignalStrength, WifiMode};
...
impl InitPlatform for OpenwrtPlatform {
    ...
    async fn is_wifi_connected(&self) -> Result<bool, InitError> {
        let wifi = self.wifi_manager().await?;
        let statuses = wifi
            .status_all()
            .await
            .map_err(|e| InitError::wifi(format!("wifi status failed: {e}")))?;
        let connected = statuses.into_iter().any(|s| {
            s.enabled
                && s.configuration
                    .as_ref()
                    .is_some_and(|cfg| cfg.mode == WifiMode::Station)
                && s.sta_link_state
                    .is_some_and(|link| link.signal_strength() != SignalStrength::Offline)
        });
        Ok(connected)
    }
    ...
    async fn ip_address(&self) -> Option<IpAddr> {
        ...
        if let Ok(name) = self.wifi_manager().await.ok()?.get_wifi_device_name().await {
            if let Some(ip) = NetworkInterface::get_by_name(&name).and_then(|n| n.ipv4_address()) {
                return Some(ip);
            }
        }
    ...
    async fn activate_generation(&self, profile_dir: &Path) -> Result<(), InitError> {
        ...
        profile::activate_profile(profile_dir, number, &path)
            .await
