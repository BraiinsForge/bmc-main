...
use bmc_render::colors::{BLACK, GRAY_50, VIOLET_60, WHITE};
use bmc_render::tree::{
...
#[must_use]
pub fn phase_label(phase: Option<UpgradePhase>) -> &'static str {
    match phase {
        None => "Preparing update",
        Some(UpgradePhase::FirmwareDownloading) => "Downloading firmware",
        Some(UpgradePhase::FirmwareVerifying) => "Verifying firmware",
        Some(UpgradePhase::FirmwareApplying) => "Applying firmware",
        Some(UpgradePhase::PackageRealizing) => "Downloading packages",
        Some(UpgradePhase::PackageVerifying) => "Verifying packages",
        Some(UpgradePhase::PackageBuilding) => "Building packages",
        Some(UpgradePhase::PackageActivating) => "Activating packages",
    }
}
...
mod tests {
    ...
    fn every_phase_has_a_stable_label() {
        assert_eq!(phase_label(None), "Preparing update");
        for phase in [
            ...
            assert!(!phase_label(Some(phase)).is_empty());
        }
    }
