...
pub fn pack_mesh(
    ...
    let tangents: Option<Vec<[f32; 4]>> = reader.read_tangents().map(std::iter::Iterator::collect);
    ...
    // indices.len() is always a multiple of 3 (triangulated mesh)
    ...
    let compressed_texture = if compress_texture {
        texture_data.as_ref().map(compress_to_etc1)
    } else {
        texture_data.as_ref().map(|t| t.data.clone())
    };
    let compressed_normal_map = normal_map_data.as_ref().map(compress_to_etc1);
...
fn extract_face_normals(document: &gltf::Document) -> Vec<[f32; 3]> {
    let Some(node) = document.nodes().next() else {
        return Vec::new();
    };
    ...
    let face_count = match json.get("face_count").and_then(serde_json::Value::as_u64) {
        Some(c) => c as usize,
        None => return Vec::new(),
    };
    let Some(flat) = json.get("face_normals").and_then(|v| v.as_array()) else {
        return Vec::new();
    };
...
fn quantize_position(val: f32, min: f32, max: f32) -> i16 {
    ...
    // Symmetric with `dequantize_position`: full i16 range,
    // endpoints round-trip exactly (t=0 ↔ i16::MIN, t=1 ↔ i16::MAX).
    let q = t * 65_535.0 - 32_768.0; // -32768..32767
    q.round() as i16
}
