...
export interface WifiProps extends SVGProps<SVGSVGElement> {
    size?: number;
    state: WifiState;
}
...
export function Wifi(props: WifiProps) {
    const { state, size, ...rest } = props;

    switch (state) {
        case 'scanning':
            return <Tick intervalMs={300} render={n => <Wifi {...rest} size={size} state={getScanFrame(n)} />} />;
