...
impl LedSceneManager {
    ...
    ///
    /// A widget id never moves between scenes: the config API mints a fresh
    /// id on add and re-ids on clone, with no move operation. The mapping
    /// thus only ever gains or loses ids, never reassigns one — a
    /// `debug_assert` guards that invariant so a future move RPC cannot
    /// silently leave an effect stranded on the old scene.
        ...
        for (instance_id, scene_id) in snapshot {
            debug_assert!(
                self.widget_to_scene
                    .get(&instance_id)
                    .is_none_or(|prev| *prev == scene_id),
                "BUG: widget {instance_id} changed scene without a removal; \
                 action routing assumes a widget id stays in one scene for its life",
            );
