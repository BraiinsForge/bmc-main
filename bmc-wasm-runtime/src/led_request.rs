...
//!
//! `LedEffect` and its protocol-aligned discriminants live in
//! `bmc-wasm-protocol`; this module just adds the request id type,
//! the per-guest allocator, and the `LedRequest` enum that the
//! runtime publishes on the host-side channel.
