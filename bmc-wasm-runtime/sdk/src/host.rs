...
mod fit_and_scale_tests {
    ...
    fn medium_canonical_fits_at_one() {
        // 638x238 is Medium's canonical box, so fit() = 1.0. This is exactly why
        // the round face must NOT use fit(): its single 390 dial renders at only
        // ~193px here, so fit()=1.0 would size the dial text ~2x too large. Round
        // uses its own min(w,h)/480 = 0.496 instead (see analog/round.rs).
