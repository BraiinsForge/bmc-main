...
///
/// `Unavailable` and `Failed` both mean "no value", and a widget
/// is free to draw them alike — but only `Failed` says asking
/// has already been tried, so waiting will not help.
///
/// A screen that draws its loading state for both leaves
/// a source that failed before its first answer loading forever.
...
impl<T> Availability<T> {
    ...
    /// Record that asking failed — unless a value has already arrived,
    /// which stands: one bad answer is worth less than the data it would blank.
    /// Reports whether this changed anything, so a caller can spare
    /// a redraw for the failure it already knows about.
