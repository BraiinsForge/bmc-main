...
impl Registry {
    ...
    pub(crate) fn request_retry(&mut self, handle: Handle) {
        self.polls[handle.0].force_retry = true;
    }

    pub(crate) fn reschedule(&mut self, handle: Handle, ok: bool, backend: &mut dyn FetchBackend) {
        let idx = handle.0;
        let ok = ok && !std::mem::take(&mut self.polls[idx].force_retry);
        if !self.polls[idx].enabled || self.polls[idx].live.is_some() {
            return;
        }
        let delay = if ok {
            match self.polls[idx].config.interval_ms {
                Some(ms) => ms,
                None => return,
            }
        } else {
            self.polls[idx].config.retry_ms
        };
