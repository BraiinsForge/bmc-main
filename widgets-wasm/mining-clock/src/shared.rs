...
//! Shared infrastructure for the analog-round renderer: palette,
//! tz helpers, the alarm-row drawer, numeric utilities, and the
//! typography-knob mapping.
...
// ── Palette ────────────────────────────────────────────────────────────
// One set of colours per day/night palette.
...
pub(crate) fn alarm_row_draws(
    ...
    let AlarmAnchor::RightX(rx) = anchor;
    let x = rx - total_w;
    #[expect(
