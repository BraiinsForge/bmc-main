...
pub(crate) fn parse_difficulty_stats(json: &impl JsonLookup) -> ParseResult<DifficultyStats> {
    // A count outside the epoch is no position in it, so it reads as missing
    // rather than as progress below zero or past the end.
    let block_epoch = json
        .f64("/block_epoch")
        .filter(|epoch| (0.0..=BLOCKS_PER_EPOCH).contains(epoch));
...
mod tests {
    ...
    /// clamping would render it as fresh progress.
    #[test]
    fn a_count_outside_the_epoch_is_refused() {
