        ...
        match &cached.snapshot.state {
            UpgradeDisplayState::Running {
                ...
                if let Some(progress) = progress {
                    let (downloaded_bytes_hi, downloaded_bytes_lo) =
                        split_u64(progress.downloaded_bytes);
                    if let Some(total) = progress.total_bytes {
                        let (total_bytes_hi, total_bytes_lo) = split_u64(total);
                        events.push(WireEvent::DownloadProgressWithTotal {
                            downloaded_bytes_hi,
                            downloaded_bytes_lo,
                            total_bytes_hi,
                            total_bytes_lo,
                        ...
                        events.push(WireEvent::DownloadProgress {
                            downloaded_bytes_hi,
                            downloaded_bytes_lo,
                        });
                    }
...
fn fan_out_resources<T>(
    resources: &mut Vec<T>,
    mut is_alive: impl FnMut(&T) -> bool,
    mut emit: impl FnMut(&T),
) {
    resources.retain(|resource| is_alive(resource));
...
impl UpgradeState {
    pub fn set(&mut self, snapshot: UpgradeDisplaySnapshot, now: Instant) {
        ...
        if let Some(events) = self.cache.events(now) {
            fan_out_resources(&mut self.resources, Resource::is_alive, |resource| {
    ...
    for event in events {
        match event {
            ...
            WireEvent::DownloadProgress {
                downloaded_bytes_hi,
                downloaded_bytes_lo,
            } => resource.download_progress(*downloaded_bytes_hi, *downloaded_bytes_lo),
            WireEvent::DownloadProgressWithTotal {
                downloaded_bytes_hi,
                downloaded_bytes_lo,
                total_bytes_hi,
                total_bytes_lo,
            } => resource.download_progress_with_total(
                *downloaded_bytes_hi,
                *downloaded_bytes_lo,
                *total_bytes_hi,
                *total_bytes_lo,
...
mod tests {
    ...
    #[test]
    fn unknown_total_progress_emits_only_downloaded_words() {
        let now = Instant::now();
        let mut cache = UpgradeCache::default();
        cache.set(
            UpgradeDisplaySnapshot {
        ...
        assert_eq!(
            ...
            Some(vec![
                WireEvent::Started(Kind::Packages),
                WireEvent::DownloadProgress {
                    downloaded_bytes_hi: 1,
                    downloaded_bytes_lo: 0,
                },
                WireEvent::SnapshotDone,
            ])
        );
    ...
    #[test]
    fn resource_fan_out_prunes_dead_entries_and_reaches_every_live_entry() {
        let mut resources = vec![1, 2, 3];
        let mut emitted = Vec::new();
        fan_out_resources(
            &mut resources,
            |resource| *resource != 2,
            |resource| emitted.push(*resource),
        );
        assert_eq!(resources, vec![1, 3]);
