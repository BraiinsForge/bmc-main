...
/// Clamp transitional states (Entering/Leaving) to Visible on the
/// initial emission: those states are only meaningful as deltas from
/// a prior Visible/Prepared, which the widget never saw.
fn clamp_initial_lifecycle(state: LifecycleState) -> LifecycleState {
    match state {
        LifecycleState::Entering | LifecycleState::Leaving => LifecycleState::Visible,
        LifecycleState::Dormant | LifecycleState::Prepared | LifecycleState::Visible | _ => state,
    }
}
...
mod tests {
    use super::{
        Emission, LifecycleSink, LifecycleState, RedrawState, dispatch_timeout,
        emit_lifecycle_batches,
    };
