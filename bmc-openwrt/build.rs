...
fn main() {
    // Lets a dev shell point unit-test builds at its compositor libraries
    // without putting those on `LD_LIBRARY_PATH`; unset elsewhere, so a no-op.
    // Unscoped deliberately: `-tests` errors on this bin-only crate
    // (no integration-test target) and `-bins` skips test-mode bin builds.
