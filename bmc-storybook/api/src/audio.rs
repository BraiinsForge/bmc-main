...
//! Rodio-backed audio sink for storybook sound playback.
//!
//! Lives in `bmc-storybook-api` so the storybook binary and the hot-reload
//! cdylib share a single canonical impl — they can't share private modules,
//! so any `audio.rs` private to either side would drift.
...
/// sink falls back to silent — `play_sound` becomes a no-op and a single very
/// loud error is emitted at construction so the developer knows why.
        ...
        let (stream, handle) = match rodio::OutputStream::try_default() {
            ...
            Err(e) => {
                tracing::error!(
                    "\n\
                     ╔══════════════════════════════════════════════════════════════════╗\n\
                     ║  STORYBOOK AUDIO DISABLED                                        ║\n\
                     ║  rodio could not open a default audio output ({e})\n\
                     ║  Sound playback is silent for this session. Fix your audio       ║\n\
                     ║  backend (PulseAudio / PipeWire / CoreAudio) and restart.        ║\n\
                     ╚══════════════════════════════════════════════════════════════════╝"
                );
                (None, None)
            }
