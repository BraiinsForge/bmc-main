...
/// Collects Nix profile diagnostics for the profile directory it is
/// constructed with. The directory is project-specific and supplied by the
/// registering binary.
#[derive(Debug)]
pub struct NixProfileExtension {
    profile_dir: &'static str,
}

impl NixProfileExtension {
    #[must_use]
    pub const fn new(profile_dir: &'static str) -> Self {
        Self { profile_dir }
    }
}

impl SupportExtension for NixProfileExtension {
    fn name(&self) -> &'static str {
        "nix_profile"
    }

    fn collect(&self, archive: &mut SupportArchive<'_>) -> Result<()> {
        collect_nix_profile(Path::new(self.profile_dir), archive)
    }
}
...
fn collect_nix_profile(profile_dir: &Path, archive: &mut SupportArchive<'_>) -> Result<()> {
    let state = read_nix_profile_state(profile_dir);
    archive.add_builtin("nix_profile_state", &state.summary)?;

    for manifest_path in state.manifests {
        match archive.add_fs_file(&manifest_path) {
...
mod tests {
    ...
    fn nix_profile_archive(profile_dir: &Path) -> BTreeMap<String, Vec<u8>> {
        ...
        collect_nix_profile(profile_dir, &mut archive).expect("BUG: collect nix profile");
