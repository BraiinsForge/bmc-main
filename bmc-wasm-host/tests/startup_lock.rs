...
fn release_lock_fd_must_point_at_selected_lockfile() {
    ...
    validate_release_lock_fd(lock_file.as_raw_fd(), &lock)
        .expect("BUG: fd pointing at lockfile should validate");
    let err = validate_release_lock_fd(lock_file.as_raw_fd(), &other)
...
fn bind_loser_drops_release_lock_and_reports_existing_host() {
    ...
    let lock_file = OpenOptions::new()
        ...
        .open(&lock)
        .expect("BUG: lock open should succeed");
    let rc_ex = unsafe { libc::flock(lock_file.as_raw_fd(), libc::LOCK_EX) };
    assert_eq!(
        rc_ex, 0,
        "BUG: parent should hold LOCK_EX before spawning waiter"
    );
    ...
    let waiter = std::thread::spawn(move || {
        ...
        let rc = unsafe { libc::flock(lock.as_raw_fd(), libc::LOCK_SH) };
        assert_eq!(
            rc, 0,
            "waiter should acquire shared lock after release-lock-fd drop"
        );
    });
