...
pub async fn fetch_and_merge_indexes(
    ...
    let enabled_servers: Vec<&ServerEntry> = servers.iter().filter(|s| s.enabled).collect();

    let primary_results: Vec<(String, PackageIndex)> =
        try_join_all(enabled_servers.iter().map(|server| async move {
            let index = fetch_index(client, &server.index_url).await?;
            Ok::<_, FetchIndexesError>((server.id.clone(), index))
        }))
        .await?;
    ...
    let mut visited: HashSet<String> = enabled_servers
        .iter()
        .map(|s| s.index_url.clone())
        .collect();

    for (server_id, index) in primary_results {
        ...
        while let Some(url) = federated_urls.pop() {
            ...
            all_fetched.push((server_id.clone(), federated_index));
        }
    }
...
pub fn merge_indexes(
    ...
    for (server_id, index) in fetched {
        ...
        for entry in &index.packages {
            ...
            let (cache_url, cache_name) = match resolve_cache(entry, &index.caches) {
                Ok(resolved) => (resolved.url, resolved.name),
                Err(e) => {
                    warn!(
                        package = %entry.name,
                        server = %server_id,
