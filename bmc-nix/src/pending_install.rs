...
/// Widget install intent carried across a firmware upgrade. Written by
/// bmc when a firmware upgrade starts with a pending install; read by
/// `bmc-nix-cli upgrade --install-from` on the far side of the flash.
