...
/// consumed, so a consumer can raw-echo exactly the lines this returns
/// `false` for.
///
/// The two axes a newer emitter can extend are handled asymmetrically, on
/// purpose. An unknown event `type` fails to parse, so this returns `false`
/// and the line surfaces as raw output — a payload whose envelope we cannot
/// even classify (foreign line, corruption) should be visible, not swallowed.
/// A recognized `Phase`/`GcPhase` carrying an unknown phase name parses fine
/// and returns `true`; `drive` drops it, so a newer emitter's unmapped phase
/// stays suppressed rather than leaking `@bmc {…}` JSON into operator output.
...
mod tests {
    ...
    #[test]
    fn parse_line_rejects_non_bmc_and_malformed_input() {
        // Interleaved fwtool/validation noise carries no prefix and must be
        // ignored rather than misparsed.
        assert!(parse_line("").is_none());
        assert!(parse_line("fwtool: writing image").is_none());
        // Prefixed but not a valid event: empty payload, truncated JSON,
        // and non-JSON all fall through to `None` (serde error), never panic.
        assert!(parse_line(BMC_PREFIX).is_none());
        assert!(parse_line("@bmc {\"type\":").is_none());
        assert!(parse_line("@bmc not json").is_none());
        // Out-of-range for the field's type: `total_paths` is unsigned, so a
        // negative value is rejected, not silently wrapped.
        assert!(parse_line("@bmc {\"type\":\"realization_started\",\"total_paths\":-1}").is_none());
        // An unrecognized event `type` is dropped like the unknown-field case
        // above: forward-compatible, not an error.
        assert!(parse_line("@bmc {\"type\":\"from_the_future\"}").is_none());
    }

    #[test]
    fn parse_line_strips_trailing_cr_and_lf() {
        // The firmware path reads these lines off a child's stderr, which may
        // arrive CRLF-terminated; `trim_end` must not defeat the prefix match.
        let line = ProgressEvent::RealizationFinished.to_bmc_line();
        assert!(parse_line(&line).is_some());
        assert!(parse_line(&format!("{line}\n")).is_some());
        assert!(parse_line(&format!("{line}\r\n")).is_some());
    }
