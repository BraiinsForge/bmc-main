...
/// Create an activation entrypoint that atomically switches
/// the `current` symlink (mimics what hook_activation_resolver generates).
...
async fn gc_removes_old_generations_during_upgrade() {
    ...
    for gen_num in 1..=3 {
        ...
        bmc_nix::profile::activate_profile(&profile_dir, &generation)
            .await
