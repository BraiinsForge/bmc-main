...
└── src/
    ...
    ├── activation.rs           # Activation script resolution (topological sort), execution
...
pub struct PackageEntry {
    ...
    pub min_bos_version: Option<String>, // YY.MM (not semver), e.g. "26.01"
    pub min_bmc_version: Option<String>, // semver, e.g. "1.0.0"
...
pub struct ManifestPackage {
    ...
    pub pinned: PinStrategy,        // false, "major", "minor", "patch"
}
...
pub enum RunActivationError {
    ...
    #[error("activation order resolution failed: {0}")]
...
pub enum ActivateProfileError {
    #[error("activation entrypoint failed: {0}")]
    Entrypoint(std::io::Error),
}
...
pub async fn build_profile(
    ...
    let profile_name = profile_dir
        ...
        .unwrap_or("profile");
    let gen_name = format!("{profile_name}-{gen_number}-link");
    ...
    // 2. Run hooks (file mergers, symlinker, activation resolver)
    hooks::run_hooks(&tmp_path, hooks_dir_name).await?;
...
/// current profile symlink as part of the activation script itself.
pub async fn activate_profile(
    ...
    let profile_name = profile_dir
        ...
        .and_then(|name| name.to_str())
        .unwrap_or("profile");
    let gen_name = format!("{profile_name}-{generation}-link");
    let gen_path = profile_dir.join(&gen_name);
    let entrypoint = gen_path.join("activation/entrypoint");
    let status = tokio::process::Command::new(entrypoint)
        .env("BMC_NEW_GENERATION", &gen_path)
        .env(
            "BMC_OLD_GENERATION",
        ...
        .status()
        .await
        .map_err(ActivateProfileError::Entrypoint)?;
    if !status.success() {
        return Err(ActivateProfileError::Entrypoint(std::io::Error::new(
...
pub async fn run_hooks(
    ...
    let hooks_dir = new_gen_path.join(hooks_dir_name);
    if !hooks_dir.exists() { return Ok(()); }

    let mut entries = sorted_dir_entries(&hooks_dir).await?;
    for entry in entries {
        // Hooks are executables; non-executable entries are not supported.
        let mut cmd = tokio::process::Command::new(entry.path());
        cmd.env("BMC_NEW_GENERATION", new_gen_path);
...
and BOS/BMC compatibility checks. Activation is optional via a flag.
...
/// package names, and checking BOS/BMC compatibility beforehand.
...
pub async fn apply_profile_change(
    ...
    store::copy_store_paths(packages).await?;
    ...
    // 4. Run activation entrypoint (optional)
    if activate {
...
`semver` is used for `min_bmc_version` only; `min_bos_version` uses a YY.MM comparator.

## Hooks as Separate Binaries

For the common hooks (file-merger, file-symlinker, activation-resolver):
...
Later, you can:
- Add a gRPC service in `bmc-grpc/proto/web/nix.proto` that calls these functions
- Add a CLI binary (`bmc-nix-cli`) that also calls these same functions
- The library stays the same
...
| Activation script resolution | `bmc-nix/src/activation.rs` |
