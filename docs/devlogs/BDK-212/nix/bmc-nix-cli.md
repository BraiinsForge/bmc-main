...
These are not needed now but are natural additions:

- `activate` — run activation scripts for a given generation
- `list-generations` — list available profile generations
- `rollback` — switch to a previous generation
- `gc` — run garbage collection

These would all be thin wrappers around the corresponding
`bmc_nix::*` functions.
