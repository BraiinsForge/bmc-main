...
#   3. produce a detached signify signature (compatible with OpenWrt
#      usign) as `<tarball>.sig` in CWD
#
# The tarball file itself is left untouched; the device downloads the
# tarball and the .sig file side-by-side and verifies with usign -V.
#
...
#               `path:<script_dir>/../../..`)
#
# Self-contained: if signify is not in PATH, the script re-executes
# itself inside `nix develop <script_dir>`, which provides signify via
# the sibling flake.nix.
set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

if [ -z "${_BDK377_FW_SIGN_IN_SHELL:-}" ] \
    && ! command -v signify >/dev/null 2>&1; then
