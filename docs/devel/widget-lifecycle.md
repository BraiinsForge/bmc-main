...
- pre-rendered as an immediate neighbour;
...
The primary consumer today is the WASM host. It uses lifecycle state to acquire and release render targets and to decide
which widgets may animate. Native widgets may also consume the event through `WidgetEvent::Lifecycle`, but widgets that
do not care about lifecycle can ignore it.
...
If the current tracker state would be `Entering` or `Leaving`, the compositor clamps the initial event to `Visible`.
Transitional states only make sense after the widget has already observed a prior idle state; a brand-new client cannot
interpret "entering" or "leaving" as a delta from anything it has seen.
...
`Prepared -> Visible` keep the same broad resource class, so they do not need release ordering.
