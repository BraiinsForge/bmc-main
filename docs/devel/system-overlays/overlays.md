...
Its `LayerConfig` is built by hand: `Layer::Overlay`, anchored to all four edges, **full** input region (so a tap below
the panel dismisses it and scene swipes are blocked while it is up). `screen_edge()` returns `ScreenEdge::Top` and
`uses_settings()` is `true`.
...
- an **upward swipe** that starts on the panel, travels up at least `DISMISS_DY` (60 px), and is mostly vertical —
  classified in `dismiss.rs`, distinct from the horizontal slider drag;
- a **tap below the panel** (start below the panel, movement within `TAP_SLOP` = 12 px);
