...
| `get_auto_hide_screen_edge(id, border, surface)` | request | `id: new_id`, `border: uint(enum)`, `surface: wl_surface` | `invalid_border` for an out-of-range border; `invalid_role` unless `surface` already has the layer-surface role. |

The `border` enum is `top = 1`, `bottom = 2`. A top reveal is a downward gesture; a bottom reveal an upward one. The
`error` enum is `invalid_border`, `invalid_role`, `already_constructed`.
