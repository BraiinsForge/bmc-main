...
| `on_device_state` / `on_setup_progress` / `on_access_point`                                                | no       | Device lifecycle, setup-flow, and setup-AP events, delivered before `tick`. One latest-wins slot per event kind (the bind replay sends all three).   |
