...
| `uses_upgrade() -> bool`                                                                                   | no       | Whether to bind `deck_upgrade_v1`; gates the upgrade hooks below.                                                                                                                                                                                      |
...
| `on_upgrade_cleared`                                                                                       | no       | Clears the upgrade presentation before `tick`; delivered under the same `uses_upgrade()` gate.                                                                                                                                                         |
...
| `uses_platform() -> bool`                                                                                  | no       | Whether to bind `deck_platform_v1`; gates the two hooks below.                                                                                                                                                                                         |
...
| `on_platform_product_name(name)`                                                                           | no       | The product's display name ("Braiins Deck", "Braiins Mini Miner"), delivered once right after the capabilities; a v1 compositor never sends it. Copy addresses the user by it; behaviour is gated on the capabilities, never on this string.           |
