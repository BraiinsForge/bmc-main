...
- **NULL buffer (unmap)** — release the old buffer and invalidate its texture, but **preserve `last_geometry`** so the
  renderer can damage the vacated region on the next frame.
...
Hiding an overlay must repaint where it was. The unmap deliberately damages the vacated region (the overlay's last
bounds, or a full-output fallback when no rect is available) so the scene shows again with no stale overlay pixels. The
partial bottom-right indicator is the sharp case: hiding it must repaint its corner, not merely stop drawing it.
...
edge. Each session carries an `EdgeFlags { border, armed, revealed }`:
