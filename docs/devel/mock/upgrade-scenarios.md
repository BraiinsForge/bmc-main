...
## Firmware success goes dark

On a real device a successful firmware upgrade ends with the process dying under sysupgrade — the stream just stops. The
mock simulates this: about 2 s after the apply hand-off it logs `Mock sysupgrade: exiting to simulate the reboot` and
exits with status 0. Use the supervisor recipe to get the reboot-and-come-back experience:
...
sequences, stream errors for the failure runs, and the radio-silence test asserting the stream breaks and the process
exits after `FirmwareApplying`.
