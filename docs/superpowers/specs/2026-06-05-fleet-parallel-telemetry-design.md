...
- **Call-path (runtime integration):** a test installs a recording `fetch_interceptor` (via `RuntimeConfig`, as the existing runtime tests do) that returns canned responses and logs each `(method, url)`. Because the interceptor returns immediately and is keyed only by `method + url` (`bmc-wasm-runtime/src/bin/capture/run.rs` strips `Fetch` `at_ms`), this verifies request **sequencing and end-state**, not timing: a cold BOS device logs in before its three-endpoint burst; an expired token triggers exactly one re-auth round re-firing only the auth-failed endpoints; a device whose endpoints all return `status: 0` is marked unreachable while the other families' devices still resolve in the same replay.
