...
Instead, a single module-level scheduler owns every `request_frame_after` call. After each `on_frame` tick (and after `ensure_running`), it asks all three drivers for their next-wake and issues exactly one `request_frame_after(min)` over the contributing families. Because there is a single writer recomputing the minimum every frame, last-write-wins is correct.

A driver contributes a timer **only when it is between passes** — counting down `PASS_INTERVAL_MS` from its last completion. Its next-wake is the remaining ms until that pass is due. A driver that is **mid-pass and awaiting fetches contributes no timer at all**: an in-flight fetch is not woken by `request_frame_after`, it is delivered through the runtime's pending-fetch channel, which fires the response callback that advances the cursor with no frame involved. Returning `0` (or any timer) for an awaiting driver would busy-render for up to the full 10 s fetch timeout — `next_frame_delay`'s contract is explicitly "schedule one timer, not busy-wait" (`bmc-wasm-runtime/src/runtime/backend.rs`). So if every family is mid-pass, the scheduler arms nothing and the next wake comes from fetch delivery; if all three are idle/waiting, it arms the soonest pass-due time. Drivers may still call `request_frame()` (immediate) to re-render on a telemetry update — that drives rendering, not pass scheduling.

A slow BOS pass cannot stretch the uBOS or Bitaxe cadence, because each family's between-pass next-wake is computed from its own completion time, and a mid-pass BOS family contributes no timer that could displace theirs.
...
The isolation guarantee is verified by **structure plus pure-logic unit tests**, not by a wall-clock integration test — the existing harness cannot model a fetch that occupies the timeout (see below). The branching logic is therefore factored into pure functions so it is testable off-wasm.

- **Pure, host-unit-testable:**
  - `DeviceList::ids_for_family` filters to the requested family (add to `device.rs`).
  - The Phase-2 decision is a pure function — given the gathered per-endpoint outcomes and the `reauthed` flag, it returns finalize vs. re-auth-and-refire(set-of-endpoints) — tested directly, the way `pass_reachable` already is. Locks in the one-login-per-pass guarantee.
  - The scheduler's next-wake is a pure function over the three drivers' pacing states — `min` of the between-pass remainders, and **no timer** for any mid-pass (awaiting-fetch) driver. This is the function that encodes per-family isolation: a test asserts that a mid-pass BOS state contributes nothing and so cannot displace a uBOS/Bitaxe between-pass wake. This is the direct, deterministic test of the "slow family does not stretch the others' cadence" property.
- **Call-path (runtime integration) — follow-up, not this slice:** a recording `fetch_interceptor` test could assert request **sequencing and end-state** (cold BOS device logs in before its three-endpoint burst; an expired token triggers exactly one re-auth round re-firing only the auth-failed endpoints). The interceptor returns immediately and is keyed only by `method + url` (`bmc-wasm-runtime/src/bin/capture/run.rs` strips `Fetch` `at_ms`), so it can only check sequencing, never timing. It also needs a harness that loads the **built** fleet-management widget into `WasmWidgetRuntime` — today's `bmc-wasm-runtime/tests/*` use WAT probe modules, not real widgets — which is unverified infrastructure. This is therefore scoped as a follow-up, alongside the deferrable-fetch harness below; the decision logic it would cover is already locked in by the pure unit tests above.

**Known gap.** No existing harness can hold a fetch open for the 10 s timeout deterministically (the interceptor sends to the completed-fetch channel immediately; `at_ms` is ignored for fetches). A faithful wall-clock test of "a dead BOS device stalls only BOS" would need new infrastructure — a deferrable fetch dispatcher that releases a response on command plus virtual-clock advance. That infra is **out of scope** for this slice; the stall isolation is instead guaranteed by the independent-driver structure and proven by the scheduler unit test above. Building the deferrable-fetch harness is noted as a possible follow-up if integration-level proof is later wanted.
