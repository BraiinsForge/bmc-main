...
- A reading that has not loaded shows `--` in its ring label and an empty ring.
- Stale readings are dropped: a value not refreshed within its freshness window clears, so the ring empties rather than
  holding an old number.
- The two data feeds — live stats (hashrate, power) and hashboards (nominal hashrate) — are independent; one going stale
