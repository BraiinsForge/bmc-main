...
The block height widget displays the latest Bitcoin block height on the Deck, with an optional readout of the block's
date and time. It fetches data from the Braiins public API and refreshes about once a minute.
...
- The height refreshes about once a minute from the Braiins public API.
...
- A *Show time and date* toggle controls whether the block's date and time appear under the height.
...
- The widget renders at the shared `small`, `medium`, `large`, and `full` sizes; the numeral and the timestamp scale per
  size.
...
- Block data comes from the Braiins public API at `public-api.braiins.com`; the widget polls roughly every 60 seconds
