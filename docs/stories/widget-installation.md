...
Widget packages that are available but not yet installed are offered right inside the "Add a widget" picker, so the user
can install a new widget without leaving the flow they are already in. Installing rides the same check → plan → run
machinery as a system upgrade: a widget install is simply a package plan that additionally pulls in the requested
widget, and it may carry an application or firmware upgrade along with it when one is needed.
