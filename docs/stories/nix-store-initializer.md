...
- The tarball is extracted to `/nix/store` and the latest profile generation is
  activated.
- If extraction does not complete successfully, the initializer does **not**
  write `nix_init=1`. On the next boot, `nix-factory-reset` still sees that
  `nix_init` is missing, removes `/nix/store`, and `bmc-nix-init` starts the
...
- TLS certificate validation is intentionally disabled because the system clock
  ...
  **NOT IMPLEMENTED** — signature verification is not yet wired up; the trust
  anchor (`FactoryServerEntry::known_public_key`) is parsed but unused, so the
  tarball is currently trusted without verification.  *Future development:*
  verify the tarball against `known_public_key` before extraction in
  `store::init_store()`, and re-enable TLS certificate validation once the
  signing pipeline is in place (tracked as #BDK-376).
...
> As a user, I want the device to recover cleanly if store extraction is
> interrupted, so that a half-extracted tarball does not leave the system in a
> broken state.

- The system boots after an interrupted or incomplete extraction.
- The early `nix-factory-reset` OpenWrt service sees that the `nix_init`
  U-Boot variable is still unset.
...
- On retry, partially downloaded store data is preserved to avoid re-downloading
  from scratch.
  **NOT IMPLEMENTED** — the retry path currently truncates the tarball and
  restarts the download from byte 0.  *Future development:* open the tarball
  in append mode when a partial file is present, issue a ranged GET
  (`Range: bytes=<offset>-`) against the factory mirror, and fall back to a
  full re-download when the server answers `200 OK` (no range support).
  Deferred until signature verification lands, since resumed bytes must still
  validate against the published digest.
